...
const MachineDetailData: React.FC = () => {
  ...
  const fetchMachineDetails = async () => {
    try {
      const response = await axios.get("/api/machinedetail");
    ...
    <div className="container mx-auto mt-4 p-4">
      <div className="flex justify-between items-center mb-4">
        <button
          ...
          className="bg-gray-700 text-white px-4 py-2 rounded hover:scale-105 transform duration-200"
        >
          Add Data
        </button>
        <input
            ...
            .map((machineDetail) => (
              <tr key={machineDetail.id} className="hover:bg-gray-100">
                ...
                <td className="py-2 px-4 border-b">
                  {machineDetail.objecttype_id}
                </td>
                <td className="py-2 px-4 border-b">
                  {machineDetail.objectgroup_id}
                </td>
                <td className="py-2 px-4 border-b">
                  {machineDetail.objectid_id}
                </td>
                <td className="py-2 px-4 border-b">
                  {machineDetail.objectcode}
                </td>
